...
//
///////////////////////////////////////////////////////////////////////////////
//! mqtt-bridge CLI - MQTT bridge for the Robonomics CPS pallet.
//!
//! This binary bridges MQTT topics and Robonomics blockchain nodes, supporting
//! subscribe, publish, and configuration-file driven operation.
...
BEHAVIOR:
    ...
    Subscribe {
        ...
        #[arg(long, default_value = "sr25519", value_parser = clap::value_parser!(CryptoScheme))]
        scheme: CryptoScheme,
    },
...
EXAMPLES:
    ...
    mqtt-bridge start --config /etc/cps/mqtt.toml
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Publish {
            ...
            commands::publish(&blockchain_config, &mqtt_config, &topic, node_id, decrypt).await?;
        }
